...
// nextTaskTracker is a minimal TaskTracker stub for testing the
// auto-continue hint path. Only NextPendingTask is parameterised;
// the other methods return zero values because the hint path doesn't
// touch them.
type nextTaskTracker struct {
	*stubTracker
	next string
}

func (n *nextTaskTracker) NextPendingTask() string { return n.next }

// nextTaskWorkspace makes nextTaskTracker satisfy both Workspace
// (via embedding promptTestWorkspace) and TaskTracker (via the
// nextTaskTracker methods).
type nextTaskWorkspace struct {
	promptTestWorkspace
	*nextTaskTracker
}

// TestRunTaskReview_AppendsNextTaskHint verifies the auto-continue
// nudge: when a TaskTracker has a pending task, the review string
...
func TestRunTaskReview_AppendsNextTaskHint(t *testing.T) {
	...
	if !strings.Contains(msg, "Next pending task:") {
		t.Errorf("review missing next-task hint; got: %q", msg)
	}
	if !strings.Contains(msg, "Implement maze rendering") {
		t.Errorf("review missing the actual task title; got: %q", msg)
	}
	if !strings.Contains(msg, "update_task") {
		t.Errorf("review missing the update_task call template; got: %q", msg)
	}
}

// TestRunTaskReview_NoHintWhenNoPending verifies the agent does NOT
// emit a hint when all tasks are done. The LLM should naturally
...
func TestRunTaskReview_NoHintWhenNoPending(t *testing.T) {
	...
	if strings.Contains(msg, "Next pending task:") {
		t.Errorf("review unexpectedly emitted next-task hint with no pending; got: %q", msg)
	}
}
