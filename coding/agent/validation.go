...
// against a candidate edit and decides whether to silently retry or
// pass through. The retry budget is per-CanonPath; budget exhaustion
// surfaces the proposal so the developer sees the failure.
//
...
func (a *Agent) runValidationPipeline(ctx context.Context, proposal EditProposal) ([]event.ValidatorSummary, string) {
	...
	if !validate.AnyNonPass(results) {
		return summaries, ""
	}

	// Non-Pass path: if the validators produced no actionable feedback,
	// there's nothing to send the LLM — surface the proposal so the
	// developer can intervene. Compute feedback BEFORE touching
	// validatorRetries: an empty-feedback round is a developer-visible
	// surface, not a silent retry, and burning a budget slot for it
