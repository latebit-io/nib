...
func TestReadFileTool_EffectIsNone(t *testing.T) {
	ws := newReadTestWorkspace(map[string]string{"f.go": "content"})
	tool := NewReadFileTool(ws, NewFileCache())

	result := tool.Execute(context.Background(), makeReadCall(t, readArgs{Path: "f.go"}))
